        ...
        for page in pdf.pages:
            ...
            for line in lines:
                date_match = re.search(
                    r'(\d{2}[-/.]\d{2}[-/.]\d{4})|(\d{4}[-/.]\d{2}[-/.]\d{2})|(\d{2}\s+[A-Za-z]{3}\s+\d{4})|([A-Za-z]{3,9}\s+\d{2},\s+\d{4})',
                    line
                )
                if date_match:
        ...
        if uploaded_file.name.lower().endswith(".pdf"):
            ...
            if df.empty:
                ...
                for m in sorted(months):
                    mtable = df_filtered[df_filtered['Month-Year'] == m][['Date', 'Amount', 'Keyword']]
                    mtable['Amount'] = pd.to_numeric(mtable['Amount'], errors='coerce').fillna(0)
                    total = mtable['Amount'].sum()
            ...
            if df_flagged.empty:
                ...
                months = df_flagged['Month-Year'].dropna().unique()
                for m in sorted(months):
                    mtable = df_flagged[df_flagged['Month-Year'] == m][['Date', 'Amount', 'Keyword']]
                    mtable['Amount'] = pd.to_numeric(mtable['Amount'], errors='coerce').fillna(0)
                    total = mtable['Amount'].sum()
                    st.markdown(f"### Transactions for {m}")
                    st.dataframe(mtable)
